...
export default class CartManager {
  ...
  async readFile() {
    try {
      ...
      console.log(`The file doesn't exist. Creating file...`);
  ...
  async updateCartById(cartId, updatedCart) {
    try {
      ...
      if (index < 0) {
        throw Error(`The cart wasn't found`);
      }
  ...
  async addProduct(cartId, productId) {
    try {
      ...
      if (!cart) {
        throw Error("The cart doesn't exist");
      }
      if (!product) {
        throw Error("The product doesn't exist");
      }
